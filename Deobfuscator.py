...
def deobfuscate_file():
    ...
    if file_path:
        ...
        deobfuscated_file, extracted_files = deobfuscator.analyze(file_path, ".")
        text_area.delete('1.0', 'end')
        text_area.insert('1.0', deobfuscated_file)
        print(extracted_files)

root = tk.Tk()
root.title("Memphis Deobfuscator")
...
title_label = ttk.Label(frame, text="Memphis Deobfuscator", font=("Helvetica", 24))
title_label.pack(side=tk.TOP, pady=10)

# Use a more descriptive button name
button = ttk.Button(frame, text="Deobfuscar Batch file", command=deobfuscate_file)
...
def show_credits():
    ...
    x = (screen_width / 2) - (window_width / 2)
    y = (screen_height / 2) - (window_height / 2)
    credits_window.geometry("%dx%d+%d+%d" % (window_width, window_height, x, y))

   
    credits_label = ttk.Label(credits_window, text="Créditos:\nMemphis\nDissectMalware", font=("Helvetica", 12))
    credits_label.pack(side=tk.TOP, pady=10)

credits_button = ttk.Button(frame, text="Ver Créditos", command=show_credits)
